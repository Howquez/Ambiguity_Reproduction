...
Ambiguity-neutral probabilities are based on finding the neo-additive probability distribution
[e.g., @Baillon2017; @Baillon_2021] with weights $w(E) = \tau + \sigma P(E)$ given to events $E$ that minimizes the Euclidian distance to the observed matching probabilities distribution. @LiTurmunkhWakker_2019.
show that $\sigma = 1-a$, $\tau = b - (1-a)/2$ and that the ambiguity-neutral probabilities $(p_1,p_2,p_3)$
are given by the following expression (see their equation (3.3)):
...
For the following analyses, we have adjusted our sample so that we meet the assumptions of @LiTurmunkhWakker_2019, namely we have removed all participants who have at least one part with $a = 1$ or violate monotonicity $(m_i ≤ m_{jk})$ at least once in part 1 or part 2. In total, this excludes 1200 participants.
...
ols_1 <- lm(formula = p1 ~ surprise + treated + surprise * treated,
  ...
  stargazer(ols_1, ols_2, ols_3, ols_4, ols_5, ols_6, 
            ...
            dep.var.caption = "Dependent variable: p1",
            dep.var.labels = " ",
            model.names = FALSE,
...
ols_1 <- lm(formula = p2 ~ surprise + treated + surprise * treated,
  ...
  stargazer(ols_1, ols_2, ols_3, ols_4, ols_5, ols_6, 
            ...
            dep.var.caption = "Dependent variable: p2",
            dep.var.labels = " ",
            model.names = FALSE,
            column.labels = c("full", "confirmation", "contradiction", "point", "interval", "both"),
            covariate.labels = c("contradiction", "both", "interval", "stage 2", "contradiction x stage 2", "interval x stage 2", "both x stage 2", "Constant"),
...
ols_1 <- lm(formula = p3 ~ surprise + treated + surprise * treated,
  ...
  stargazer(ols_1, ols_2, ols_3, ols_4, ols_5, ols_6, 
            ...
            dep.var.caption = "Dependent variable: p3",
            dep.var.labels = " ",
            model.names = FALSE,
            column.labels = c("full", "confirmation", "contradiction", "point", "interval", "both"),
            covariate.labels = c("contradiction", "both", "interval", "stage 2", "contradiction x stage 2", "interval x stage 2", "both x stage 2", "Constant"),
...
As before, we use the `{{patchwork}}` package to assmeble the plot objects.

```{r fig-OE4}
#| label: fig-OE4S
#| fig-cap: 'Treatment effects of regression equation (1) with dependent variables $p_1$, $p_2$, and $p_3$. Estimators with 95% confidence intervals. The underlying standard errors (“HC1”) are clustered at the individual level and estimated with the R package sandwich (Zeileis, 2004; Zeileis et al., 2020).'
(top / (left | right) & theme(legend.position = "bottom")) + plot_layout(guides = "collect")
```

Note that the order of both, the y-axis as well as the control variables is different compared to the Figure in the manuscript.
